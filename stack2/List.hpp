...
template <typename T>
bool cop4530::operator==(const List<T>& lhs, const List<T>& rhs) { //cheks if two lists are the same
...
template <typename T>
bool cop4530::operator!=(const List<T>& lhs, const List<T>& rhs) { //checls if two lists are not the same
	return !(lhs == rhs);
}

template <typename T>
std::ostream& cop4530::operator<<(std::ostream& os, const List<T>& l) { //prints the list
	l.print(os,' ');
	return os;
}
