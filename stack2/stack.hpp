...
template <typename T>
typename std::ostream& cop4530::operator<< (std::ostream& os, const Stack<T>& a) {
	a.print(os, ' ');
}

template <typename T>
typename bool cop4530::operator== (const Stack<T>& lhs, const Stack<T>& rhs) {
	return (lhs == rhs);
}
...
template <typename T>
typename bool cop4530::operator<= (const Stack<T>& a, const Stack<T>& b) {
	if (a.size() != b.size())
		return false;
	Stack<T> aCopy = a;
	Stack<T> bCopy = b;
